...
1. Your task is to implement functionality per the specifications using subagents. Follow @IMPLEMENTATION_PLAN.md and choose the most important item to address. Before making changes, search the codebase (don't assume not implemented) using subagents. Use subagents for searches/reads and only 1 subagent for build/tests. Use amp subagents with oracle when complex reasoning is needed (debugging, architectural decisions). Tasks include required tests - implement tests as part of task scope.
...
99999999999999. If you find inconsistencies in the specs/* then use amp subagents with oracle with 'ultrathink' requested to update the specs.
